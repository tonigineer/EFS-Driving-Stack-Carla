...
class MPCNode(CompatibleNode):
    ...
    def update_controller(self):
        ...
        self.loginfo(f'{self.reference.shape}')

        start = perf_counter()
        control_out, state_horizon = self.mpc.solve(
            state_vector=x0, reference=ref)

        self.loginfo(f'MPC - delta_v: {control_out[0][0]:+0.2f} | '
                     f'ax: {control_out[1][0]:+0.1f} - Exec.time: '
                     f'{perf_counter()-start:0.3f}ms')

        msg = AckermannDrive()
        msg.steering_angle = control_out[0][0]
        msg.acceleration = control_out[1][0]
        msg.speed = 5.0
